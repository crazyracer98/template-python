"""E2E smoke test: /v2/heroes/xml CRUD, through real Playwright requests against the live api."""
...
def test_hero_xml_crud_lifecycle(
    ...
    try:
        list_response = page.request.get(f"{base_url}/v2/heroes/xml", headers=headers)
        assert list_response.ok
        assert f"<id>{hero_id}</id>" in list_response.text()

        get_response = page.request.get(
            f"{base_url}/v2/heroes/xml", params={"id": hero_id}, headers=headers
        )
...
def test_hero_xml_bulk_update_and_delete_via_filters(
    ...
    """PATCH/DELETE /v2/heroes/xml?<filters> act in bulk and render an XML bulk-result body."""
    headers = {
        "Authorization": f"Bearer {access_token('maintainer')}",
        "Content-Type": "application/xml",
    }
