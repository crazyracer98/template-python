"""E2E smoke test: /v1/heroes CRUD, through real Playwright requests against the live api."""
...
def test_hero_v1_crud_lifecycle(
    ...
    try:
        list_response = page.request.get(f"{base_url}/v1/heroes", headers=headers)
...
def test_v1_heroes_responses_carry_deprecation_headers(
    ...
    """/v1/heroes responses carry Sunset/Deprecation/Link headers; /v2/heroes responses don't."""
    headers = {"Authorization": f"Bearer {access_token('maintainer')}"}

    v1_response = page.request.get(f"{base_url}/v1/heroes", headers=headers)
    assert v1_response.ok
    assert v1_response.headers["deprecation"] == "true"
    assert "sunset" in v1_response.headers
    assert v1_response.headers["link"] == '</v2/heroes>; rel="sunset"'

    v2_response = page.request.get(f"{base_url}/v2/heroes", headers=headers)
