...
def test_to_xml_renders_one_element_per_list_item() -> None:
    ...
    hero = HeroCreate(name="Storm", powers=["Weather control", "Flight"])
    xml = to_xml(hero, "hero")
...
def test_from_xml_parses_fields_into_the_given_schema() -> None:
    ...
    assert hero == HeroCreate(name="Batman", powers=["Detective skills"])
...
def test_from_xml_groups_repeated_elements_into_a_list() -> None:
    ...
    hero = from_xml(xml, HeroCreate)
    assert hero == HeroCreate(name="Storm", powers=["Weather control", "Flight"])
...
def test_xml_round_trips_through_to_xml_and_from_xml() -> None:
    ...
    """
    original = HeroCreate(name="Wonder Woman", powers=["Super strength", "Flight"])
